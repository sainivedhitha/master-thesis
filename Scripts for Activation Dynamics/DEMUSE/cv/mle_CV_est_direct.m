function [cv,cvCorr] = mle_CV_est_direct(Segna,dint,fsamp)
...
for k1=2:size(Segna,1); % Segna is already in direction of AP propagation, hence, no control of direction is needed !!!
    iSenga(k1,:) = [zeros(1,(100-round((k1-1)*teta))*interpFact) interp(Segna(k1,:),interpFact) zeros(1,(100+round((k1-1)*teta))*interpFact)];    
end
%plotSignalsTorino(1,0,iSenga);

tmpCC = corrcoef(iSenga');
ind = find(tmpCC<1);
cvCorr = median(tmpCC(ind)');
