function data = DEMUSEtool_reader_IaDistribution(filepath,filename,epoch_length)
...
EMG=sMAT;%(1:64,1:10240/2048:length(sMAT));

%clear sMAT;
LBoundary=300;
HBoundary=LBoundary+60;

EMG=EMG(1:64,LBoundary*2048:HBoundary*2048);

range=[LBoundary HBoundary];

if isempty(dir([filepath,'DecompRange.mat'])),
    save([filepath,'DecompRange.mat'],'range');
else
    load([filepath,'DecompRange.mat']);
    range=[range LBoundary HBoundary];
    save([filepath,'DecompRange.mat'],'range');
end

% save([filepath,'DecompRange',num2str(LBoundary),'.mat'],'range');

c=1;
for k1=12:-1:1,    
    data.SIG{c,1} = EMG(k1,:)';
%     index = find(sMAT(k1,1:end-1)>2000 & sMAT(k1,2:end)<2000);
