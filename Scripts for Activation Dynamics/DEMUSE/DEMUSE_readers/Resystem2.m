function sdl_mtx = Resystem2(sig_sdl);
...
k = 1;
for column = 2 : Col - 1
    sdl_mtx{1,column} = temp(k, :);
%     disp(['row: ' num2str(1) ' column: ' num2str(column) ' index: ' num2str(k)]);
    k = k + 1;
end
...
k = 0;
for column = 2 : Col - 1
   sdl_mtx{12,column} = temp(56 - k, :);
%    disp(['row: ' num2str(12) ' column: ' num2str(column) ' index: ' num2str(56 - k)]);
   k = k + 1;
end
