function data = DEMUSEtool_reader_trap_EMG(filepath,filename,epoch_length)
...
c=1;
for k1=12:-1:1,    
    data.SIG{c,1} = EMG(k1,:)./2^12*5;
    index = find(EMG(k1,1:end-1)>2000 & EMG(k1,2:end)<2000);
    if length(index)>1, [c,1],data.SIG{c,1}=[];end;
    c=c+1;
end
c=1;
for k1=13:1:25,
    data.SIG{c,2} = EMG(k1,:)./2^12*5;
    index = find(EMG(k1,1:end-1)>2000 & EMG(k1,2:end)<2000);
    if length(index)>1, [c,1],data.SIG{c,2}=[];end;
    c=c+1;
end
c=1;
for k1=38:-1:26,
    data.SIG{c,3} = EMG(k1,:)./2^12*5;
    index = find(EMG(k1,1:end-1)>2000 & EMG(k1,2:end)<2000);
    if length(index)>1, [c,1],data.SIG{c,3}=[];end;
    c=c+1;
end
c=1;
for k1=39:51,
    data.SIG{c,4} = EMG(k1,:)./2^12*5;
    index = find(EMG(k1,1:end-1)>2000 & EMG(k1,2:end)<2000);
    if length(index)>1, [c,1],data.SIG{c,4}=[];end;
    c=c+1;
end
c=1;
for k1=52:64,
    data.SIG{c,5} = EMG(k1,:)./2^12*5;
    index = find(EMG(k1,1:end-1)>2000 & EMG(k1,2:end)<2000);
    if length(index)>1, [c,1],data.SIG{c,5}=[];end;
    c=c+1;
end
